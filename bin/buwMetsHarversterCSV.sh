...
find "$ingestPath" -name "ie*.csv" -printf "%h\t" -printf "%p\n" | while read -r pname fname; do
     ...
     grep 'https://elekpub.bib.uni-wuppertal.de/download/.*"' "$fname" >> "$pname"/flocation.txt
...
mkdir "ie_backup"
cp ./content/ie*.csv ./ie_backup

# Einloggen und speichern von cookie
# Hier Logindaten eingeben loginUser und loginPassword
echo "Login"
curl = -d "loginUser=hbz-rosetta&loginPassword=r@setta#!" -c cookie.txt "https://elekpub.bib.uni-wuppertal.de/oai/login/" -s -A "Mozilla/4.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36;"
...
find "$ingestPath" -name "urls.txt" -printf "%h\t" -printf "%p\n" | while read -r pname urlfile; do
    ...
    while read -r url; do
        ...
        find ../ -name "ie*.csv" -execdir sed --in-place=.sed -e "$regex" {} \;
        cd "$currentdir" || exit
    done < "$urlfile"
done
